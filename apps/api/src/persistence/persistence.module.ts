...
import {
  ...
  RepositoryAccountStateProvider,
  type PortfolioRepository,
} from "@stonks/portfolio";
import type { AccountStateProvider } from "@stonks/contracts";
 ...
 * AccountStateProvider（contracts B2）は portfolio の RepositoryAccountStateProvider で
 * 構成し、trading-engine の現金/保有チェックを portfolio の状態に一致させる。
 */
@Module({
  providers: [
    {
    ...
    },
    {
      ...
      useFactory: (repo: PortfolioRepository): AccountStateProvider =>
        new RepositoryAccountStateProvider(repo),
    },
  ],
  exports: [
